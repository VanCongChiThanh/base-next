...
export const categoryService = {
  ...
  async create(data: { name: string; description?: string; icon?: string; type?: string | null }): Promise<JobCategory> {
    return apiClient.post<JobCategory>("/job-categories", data);
  },
  async delete(id: string): Promise<{ message: string }> {
    return apiClient.delete<{ message: string }>(`/job-categories/${id}`);
  },
  async update(id: string, data: { name?: string; description?: string; icon?: string; type?: string | null }): Promise<JobCategory> {
