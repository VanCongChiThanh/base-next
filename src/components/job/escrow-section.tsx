...
export function EscrowSection({ job }: EscrowSectionProps) {
  ...
  const handleSubmitMilestone = async (milestoneId: string) => {
    try {
      ...
      await paymentService.submitMilestone(
        milestoneId,
        submitNotes[milestoneId],
      );
  ...
  const handleReviewMilestone = async (
    ...
    try {
      ...
      await paymentService.reviewMilestone(
        milestoneId,
        action,
        reviewNotes[milestoneId],
      );
      setSuccess(
  ...
  const getStatusLabel = (status: MilestoneStatus) => {
    ...
    return (
      <span
        className={`px-2 py-1 text-xs font-medium rounded-full ${mapped.color}`}
      >
        {mapped.text}
      </span>
    );
  ...
  if (loading)
    return <div className="animate-pulse h-32 bg-blue-50 rounded-2xl" />;
      ...
      {!escrow && isEmployer ? (
        <div className="bg-white rounded-2xl border border-blue-100 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">
            Quản lý Thanh toán (Milestones)
          </h2>
          <p className="text-sm text-gray-500 mb-4">
            Tạo các cột mốc thanh toán và ký quỹ tiền để bắt đầu công việc.
          </p>
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="text-right">
              ...
              <p className="text-xl font-bold text-blue-600">
                {escrow.totalAmount.toLocaleString("vi-VN")}đ
              </p>
            </div>
          </div>
                ...
                {isEmployer && m.status === MilestoneStatus.SUBMITTED && (
                  <div className="mt-4 pt-4 border-t space-y-3">
                    <div className="p-3 bg-blue-50 text-blue-800 text-sm rounded-lg border border-blue-100">
                      <span className="font-semibold">Worker nộp:</span>{" "}
                      {m.submissionNote || "Không có ghi chú"}
                    </div>
                    <textarea
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() =>
                          handleReviewMilestone(m.id, "request_revision")
                        }
                        disabled={
                          actionLoading === `review-${m.id}` ||
                          !reviewNotes[m.id]
                        }
